...
template <typename T> PyArrayObject *get_numpy_array_1D(long int dim) {
  npy_intp dims[] = {dim};
  void *buffer = malloc(dim * sizeof(T));
  PyObject *narray =
      PyArray_SimpleNewFromData(1, dims, NpDType<T>::typevalue, buffer);
...
PyArrayObject *get_numpy_array_2D(long int dimX, long int dimY) {
  ...
  PyObject *narray =
      PyArray_SimpleNewFromData(2, dims, NpDType<T>::typevalue, buffer);
  PyArray_ENABLEFLAGS((PyArrayObject *)narray, NPY_ARRAY_OWNDATA);
  return (PyArrayObject *)PyArray_Return((PyArrayObject *)narray);
}

inline PyObject *unicode_pystr(std::string s) {
