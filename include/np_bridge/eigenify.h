...
EigenMatrixMapUnaligned<T> eigenify2d(NumpyArray2Dim<T> &arr) {
  ...
  constexpr int s = sizeof(T);
  return EigenMatrixMapUnaligned<T>(
      &arr.at(0, 0), arr.sizeX(), arr.sizeY(),
      Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>(arr.strides[1] / s,
                                                    arr.strides[0] / s));
}
} // namespace np_bridge
