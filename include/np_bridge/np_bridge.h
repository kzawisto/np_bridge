...
public:
  ...
  T &at(long i) { return *(T *)(data + strides[0] * i); }
  T *begin() { return this->data; }
  T *end() { return this->data + this->dimmensions[0] * strides[0]; }
  const T &c_at(long i) const { return *(T *)(data + strides[0] * i); }
  long size() const { return dimmensions[0]; }
...
template <class T> class NumpyArray2Dim : public NumpyArrWrapper {
public:
  ...
  long sizeX() const { return dimmensions[0]; }
  long sizeY() const { return dimmensions[1]; }
};
template <typename T> class NumpyArrayObjImpl : public NumpyArray1Dim<T> {
...
public:
  ...
  template <typename K>
  NumpyArrayObjImpl(const std::vector<K> &vec)
      : NumpyArrayObjImpl<T>(vec.size()) {
    for (unsigned int i = 0; i < vec.size(); ++i) {
      this->at(i) = vec.at(i);
    }
  }

  ~NumpyArrayObjImpl() { Py_XDECREF(this->arr); }
};
template <class K> class NumpyDetachable : public K {
public:
  template <typename... Args> NumpyDetachable(Args... args) : K(args...) {}
...
template <typename K> using NpArrayObj = NumpyDetachable<NumpyArrayObjImpl<K>>;

template <typename T>
class NumpyArrayObj2Dim
    : public NumpyArray2Dim<T> { //, public NumpyDetachable<NumpyArrayObj2D>{
public:
...
public:
  NumpyArrayCBuffer2D(long dimX, long dimY, bool is_c_order = true)
      : NumpyArray2DimD(nullptr), is_c_order(is_c_order) {
...
public:
  NumpyArrayCBufferView2D(char *buf, long dimX, long dimY,
                          bool is_c_order = true)
      : NumpyArray2DimD(nullptr), buffer(buf) {
    this->data = buffer;
    this->dimmensions = new npy_intp[2]{dimX, dimY};
    this->strides = get_strides_2d<double>(dimX, dimY, is_c_order);
  }
...
template <typename T> class NumpyArrayCBuffer1D : public NumpyArray1Dim<T> {
...
template <typename T> class NumpyArrayCBufferView1D : public NumpyArray1Dim<T> {
public:
  ...
  NumpyArrayCBufferView1D(NumpyArrayCBufferView1D &&buf)
      : NumpyArray1Dim<T>(nullptr) {
    this->data = buf.data;
    buf.data = nullptr;
    this->strides = buf.strides;
    buf.strides = nullptr;
...
template <typename T> class NumpyArrayView : public NumpyArray1Dim<T> {
public:
  NumpyArrayView(PyArrayObject *arr) : NumpyArray1Dim<T>(arr) {
    if (arr->descr->type != NpDType<T>::typechar) {
      throw std::logic_error(

          stringify("Got typechar ", arr->descr->type, " but expected ",
                    (char)NpDType<T>::typechar));
    }
    if (arr->nd != 1) {
      throw std::logic_error(
...
template <typename T> class NumpyArrayView2D : public NumpyArray2Dim<double> {
public:
