...
namespace np_bridge {
template<typename T>
NumpyArrayView<T> get_column_values(PyObject * pandas_dataframe, std::string key) {

        PyObject * obj = PyObject_CallMethod(pandas_dataframe, "__getitem__", "s", key.c_str());
        if(obj == nullptr) {
        ...
        return NumpyArrayView<T>(
                        (PyArrayObject*)obj
                );
 }
template<typename T>
EigenVectorMapUnaligned<T> get_eigen_col(PyObject * pandas_dataframe, std::string key) {
